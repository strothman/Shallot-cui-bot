...
class LoraBuildGridButtons(discord.ui.View):
    """Interactive action buttons attached to /lora-build generated image grids."""
    def __init__(self, generation_id: str, session_id: str):
        super().__init__(timeout=None)
        self.generation_id = generation_id
        self.session_id = session_id

        # Row 0: Add Quadrants 1-4 directly to LoRA dataset
        self.add_item(discord.ui.Button(
            label="➕ Add Q1",
            style=discord.ButtonStyle.success,
            custom_id=f"lora_add:{generation_id}:1",
            row=0
        ))
        self.add_item(discord.ui.Button(
            label="➕ Add Q2",
            style=discord.ButtonStyle.success,
            custom_id=f"lora_add:{generation_id}:2",
            row=0
        ))
        self.add_item(discord.ui.Button(
            label="➕ Add Q3",
            style=discord.ButtonStyle.success,
            custom_id=f"lora_add:{generation_id}:3",
            row=0
        ))
        self.add_item(discord.ui.Button(
            label="➕ Add Q4",
            style=discord.ButtonStyle.success,
            custom_id=f"lora_add:{generation_id}:4",
            row=0
        ))

        # Row 1: Describe & Add (Runs Florence-2 immediately on quadrant and adds)
        self.add_item(discord.ui.Button(
            label="🏷️ Describe & Add Q1",
            style=discord.ButtonStyle.primary,
            custom_id=f"lora_desc_add:{generation_id}:1",
            row=1
        ))
        self.add_item(discord.ui.Button(
            label="🏷️ Describe & Add Q2",
            style=discord.ButtonStyle.primary,
            custom_id=f"lora_desc_add:{generation_id}:2",
            row=1
        ))
        self.add_item(discord.ui.Button(
            label="🏷️ Describe & Add Q3",
            style=discord.ButtonStyle.primary,
            custom_id=f"lora_desc_add:{generation_id}:3",
            row=1
        ))
        self.add_item(discord.ui.Button(
            label="🏷️ Describe & Add Q4",
            style=discord.ButtonStyle.primary,
            custom_id=f"lora_desc_add:{generation_id}:4",
            row=1
        ))

        # Row 2: Workflow Utilities
        self.add_item(discord.ui.Button(
            label="💡 Next Shot Idea",
            style=discord.ButtonStyle.secondary,
            custom_id=f"lora_idea:{session_id}",
            row=2
        ))
        self.add_item(discord.ui.Button(
            label="📊 Dataset Status",
            style=discord.ButtonStyle.secondary,
            custom_id=f"lora_status:{session_id}",
            row=2
        ))
        self.add_item(discord.ui.Button(
            label="🔄 Reroll Grid",
...
class LoraBuildStatusView(discord.ui.View):
    """Action buttons attached to /lora-build status and management embeds."""
    def __init__(self, session_id: str):
        ...
        self.add_item(discord.ui.Button(
            ...
            style=discord.ButtonStyle.primary,
            custom_id=f"lora_suggest:{session_id}",
            row=0
        ))
        self.add_item(discord.ui.Button(
            label="🏷️ Describe All (Florence-2)",
            style=discord.ButtonStyle.primary,
            custom_id=f"lora_describe_all:{session_id}",
            row=0
        ))
        self.add_item(discord.ui.Button(
            label="📦 Export Dataset ZIP",
            style=discord.ButtonStyle.success,
            custom_id=f"lora_export:{session_id}",
            row=0
        ))
